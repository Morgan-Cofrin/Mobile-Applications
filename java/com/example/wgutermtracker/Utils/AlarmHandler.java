...
import com.example.wgutermtracker.Activities.Assessment.AssessmentViewerActivity;
import com.example.wgutermtracker.Activities.Course.CourseViewerActivity;
import com.example.wgutermtracker.Entities.Assessment;
import com.example.wgutermtracker.Entities.Course;
import com.example.wgutermtracker.Activities.Main.MainActivity;
...
public class AlarmHandler extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (destination) {
            case "course":
                Course course = DBDataManager.getCourse(context, id);
                if (course != null && (course.startNotifications == 1 || course.endNotifications == 1)) {
                    resultIntent = new Intent(context, CourseViewerActivity.class);
                    uri = Uri.parse(DBProvider.COURSES_URI + "/" + id);
                    resultIntent.putExtra(DBProvider.COURSE_CONTENT_TYPE, uri);
                }
                else {
                    return;
                }
                break;
            case "assessment":
                Assessment assessment = DBDataManager.getAssessment(context, id);
                if (assessment != null && assessment.notifications == 1) {
                    resultIntent = new Intent(context, AssessmentViewerActivity.class);
                    uri = Uri.parse(DBProvider.ASSESSMENTS_URI + "/" + id);
                    resultIntent.putExtra(DBProvider.ASSESSMENT_CONTENT_TYPE, uri);
                }
                else {
                    return;
                }
